...
/// Note: Marked @unchecked Sendable because EKEventStore is thread-safe for read operations
/// and the tool only performs short-lived operations without storing mutable state
final class CalendarTool: ClarissaTool, @unchecked Sendable {
    ...
    private func listEvents(_ args: [String: Any]) async throws -> String {
        ...
        guard let endDate = Calendar.current.date(byAdding: .day, value: daysAhead, to: startDate) else {
            throw ToolError.executionFailed("Unable to calculate end date for calendar search")
        }
