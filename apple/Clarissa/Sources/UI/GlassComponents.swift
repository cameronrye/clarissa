...
// MARK: - Shared Animated Dots Component

/// Configurable animated dots view used for typing and thinking indicators
/// Supports accessibility reduce motion and customizable appearance
struct AnimatedDotsView: View {
    ...
    var animationInterval: TimeInterval = 0.35
    /// Whether to animate scale (typing style) or just opacity (thinking style)
    ...
    private func scaleFor(_ index: Int) -> CGFloat {
        guard shouldAnimate && animateScale else { return 1.0 }
        return phase == index ? 1.2 : 0.8
    }

    private func opacityFor(_ index: Int) -> Double {
        guard shouldAnimate else { return 0.6 }
        return phase == index ? 1.0 : (animateScale ? 0.4 : 0.3)
    }
...
/// Animated thinking dots for the glass indicator - uses shared AnimatedDotsView
@available(iOS 26.0, macOS 26.0, *)
private struct ThinkingDotsView: View {
    ...
    var body: some View {
        AnimatedDotsView(
            ...
            animateScale: false,
            respectReduceMotion: false  // Glass indicators always animate
        )
    }
}
